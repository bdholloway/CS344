...
void SendFile(int fd, int sockfd) {
	//Tracks how many bytes have been read and wrote
	int BytesRead;
	int BytesWrote;
...
void HandleRequest(int sockfd, char * filename, char * keyname){
	...
	char * decrypted = RecvFile(sockfd, FileLen);
	//Print out decrypted
	printf("%s", decrypted);
    //Free mem alocated
	free(decrypted);
...
/*******************************************************************************
 * main(int, char)
 * Runs program and calls appropriate funcs
 ******************************************************************************/
int main(int argc, char *argv[]){
	//If number of argument is not 4, send msg to std error and exit
	if(argc != 4){
		fprintf(stderr, "Invalid number of arguments\n");
		fprintf(stderr, "Usage: opt_enc filename keyname portnumber\n");
		exit(1);
	}
	//Opens file, Checks for invalid chars by calling FileCheck
	int fd = open(argv[1], O_RDONLY);
    //if fd is < 0, msg stderr, then exit
	if(fd < 0){
	...
	FileCheck(fd);
    //Close up file when done
	close(fd);
	//Does same as above for other file
	...
	//Calls funcs to handle request
	HandleRequest(sockfd, argv[1], argv[2]);
	//free up struct infor that was allocated
    freeaddrinfo(res);
    //Clsoe socket
	close(sockfd);
	return 0;
